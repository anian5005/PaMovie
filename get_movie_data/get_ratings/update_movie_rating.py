...
from local_package.web_crawler_tools.multi_thread import DoubaonCookiesCrawler, MultiThread
from local_package.db.mysql import save_error_log, MySQL, update_on_duplicate_key, recheck_if_error
from local_package.fetcher.omdb_api_fetcher import multi_thread_update_tomato_rating_by_omdb_api
...
def get_imdb_list_to_update(start_year):
    sql = MySQL()
...
def update_imdb_ratings():
    ...
    # get movie id list to update
    imdb_list = get_imdb_list_to_update(2018)[0]

    # update imdb rating
    inserted_imdb_data_list, imdb_no_rating_list = get_imdb_rating(imdb_list)
    imdb_end = time.time()

    # recheck data pipeline result & error count
    imdb_condition = 'where DATE(imdb_updated) = "{}"'.format(log_timestamp_date)
    imdb_updated_list = sql.fetch_data(sql_conn, 'movie_rating', ['imdb_id'], imdb_condition, 'list')
...
def multi_thread_update_douban_ratings(worker_num, target_docs_num):
    sql = MySQL()
    engine, sql_conn = sql.get_connection()
    start = time.time()
    log_timestamp_date = date.fromtimestamp(start)
    ...
    douban_mapping_imdb_list = get_imdb_list_to_update(2018)[1]
    ...
    multi_thread_douban_crawler = DoubaonCookiesCrawler(
        id_list=douban_mapping_imdb_list,
    ...
    WHERE start_year =2022 and DATE(douban_updated) ='{}'""".format(log_timestamp_date)
    ...
    dashboard_log = {
        ...
        'douban_rating_time': round((douban_end - start), 0),
...
def multi_thread_update_rotten_tomatoes_ratings(worker_num):
    sql = MySQL()
    engine, sql_conn = sql.get_connection()
    start = time.time()
    log_timestamp_date = date.fromtimestamp(start)
    ...
    # get movie id list to update
    tomato_mapping_imdb_list = get_imdb_list_to_update(2018)[2]
    # id_dict {'imdb_id': tt55555555, 'tomato_id': 'top_gun_maverick'}

    # update tomato rating by web crawler
    multi_thread_cookies_creator = MultiThread(id_list=tomato_mapping_imdb_list,
                                               worker_num=worker_num,
    ...
    # update tomato rating by omdb api
    ...
    # recheck data pipeline result & error count
    tomatoes_condition = 'WHERE DATE(tomatoes_updated) = "{}" AND tomatoes_source="crawler" '.format(log_timestamp_date)
    tomatoes_updated_rating_list = sql.fetch_data(sql_conn, 'movie_rating', ['imdb_id'], tomatoes_condition, 'list')
    ...
    dashboard_log = {
        'log_time': log_timestamp_date,
        'rotten_tomatoes_rating_time': round((tomato_end - start), 0),
        'tomato_to_update': len(tomato_mapping_imdb_list),
        'rotten_tomatoes_error_count': tomato_crawler_error_count,
        'rotten_tomatoes_updated_count':  len(tomatoes_updated_rating_list),
